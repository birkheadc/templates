...
export default function RepeatPasswordInput<T extends { repeat: any }>(props: RepeatPasswordInputProps<T>): JSX.Element {
  ...
  const { id, disabled, register, password, errors, name, className } = props;
  ...
  return (
    <div className={utils.mergeClass('flex flex-col gap-1 w-full', className)}>
      ...
      <BaseInput autoComplete='new-password' {...props} type={show ? 'text': 'password' } register={register} registerOptions={{ validate: (value: string) => value === password || FormValidationErrorMessage.EXACT_MATCH }} />
